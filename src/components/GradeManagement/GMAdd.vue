          ...
          <div>
            <q-card class="my-card">
              <q-card-section>
                ...
                <div class="row q-gutter-md row items-start">
                  <div class="col-12 col-md">
                    <q-input dense name="percentage" type="text" v-model="data.percentage_grade" class="text-primary" label="Percentage Grade">
                      <template v-slot:append>
                  ...
                  <div class="col-12 col-md">
                    <q-input dense name="numerical" type="text" v-model="data.numerical_grade" class="text-primary" label="Numerical Grade">
                      <template v-slot:append>
                        <q-icon color="black" name="grade" />
                      </template>
                    </q-input>
...
export default {
  ...
  methods: {
    submit () {
      ...
      self.$store.commit('GradeManagement/setPost', self.data)
      self.$store.dispatch('GradeManagement/createData')
        .then(res => {
          // self.showNotif()
          self.$store.commit('GradeManagement/pushList', res.data)
          self.reset()
    ...
    reset () {
      var self = this
      self.$store.commit('GradeManagement/setPost', '')
      self.$emit('close')
    },
    showNotif () {
    },
