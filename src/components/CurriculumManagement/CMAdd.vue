...
export default {
  ...
  methods: {
    submit () {
      ...
      self.$store.commit('CurriculumManagement/setPost', self.data)
      self.$store.dispatch('CurriculumManagement/createData')
        .then(res => {
          // self.showNotif()
          self.$store.commit('CurriculumManagement/pushList', res.data)
    ...
    reset () {
      var self = this
      self.$store.commit('CurriculumManagement/setPost', '')
      self.$emit('close')
    },
    showNotif () {
    }
