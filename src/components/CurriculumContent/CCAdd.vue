...
export default {
  ...
  methods: {
    submit () {
      ...
      self.$store.commit('CurriculumContent/setPost', self.data)
      self.$store.dispatch('CurriculumContent/createData')
        .then(res => {
          // self.showNotif()
          self.$store.commit('CurriculumContent/pushList', res.data)
          self.reset()
    ...
    reset () {
      var self = this
      self.$store.commit('CurriculumContent/setPost', '')
      self.$emit('close')
    },
    showNotif () {
    },
