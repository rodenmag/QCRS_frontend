...
export default {
  ...
  created () {
    ...
    self.delete_data = JSON.parse(JSON.stringify(self.$store.getters['SponsorPayment/getDelete']))
    // console.log(self.delete_data)
  },
  methods: {
    Delete () {
      var self = this
      self.$store.commit('SponsorPayment/setDelete', self.delete_data)
      self.$store.dispatch('SponsorPayment/deleteData')
        .then(res => {
          ...
          self.$store.commit('SponsorPayment/deleteList', self.delete_data)
        }).catch(err => {
          console.log(err)
        })
    }
