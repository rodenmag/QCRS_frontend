...
export default {
  ...
  async created () {
    var self = this
    self.data = JSON.parse(JSON.stringify(self.$store.getters['SubjectManagement/getPut']))
  },
  methods: {
    submit () {
      var self = this
      self.data.posted_by = self.$store.state.user_info.id
      self.$store.commit('SubjectManagement/setPut', self.data)
      self.$store.dispatch('SubjectManagement/updateData')
        .then(res => {
          // self.showNotif()
          self.$store.commit('SubjectManagement/modifyList', res.data)
    ...
    reset () {
      var self = this
      self.$store.commit('SubjectManagement/setPut', '')
      self.$emit('close')
    },
    showNotif () {
    }
