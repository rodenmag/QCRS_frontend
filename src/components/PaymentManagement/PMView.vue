...
export default {
  ...
  async created () {
    var self = this
    self.student_data = JSON.parse(JSON.stringify(self.$store.getters['PaymentManagement/getData']))
    self.student_name = self.student_data.full_name
    self.getCreated()
  },
  watch: {
  ...
  methods: {
    getCreated() {
      var self = this
      self.data_list = []
      self.$store.dispatch('PaymentManagement/searchData')
        .then(res => {
          self.$store.commit('PaymentManagement/setList', res.data)
          self.data_list = self.$store.getters['PaymentManagement/getList']
          self.$store.commit('PaymentManagement/setBalance', self.data_list.reduce((n, {debit}) => n + debit, 0) - self.data_list.reduce((n, {credit}) => n + credit, 0))
          self.balance = self.$store.getters['PaymentManagement/getBalance'].toString()
        }).catch(err => {
          console.log(err)
        })
    },
    getPayload (data) {
      var self = this
      self.$store.commit('PaymentManagement/setPut', data)
    },
    getPayloadDelete (data) {
      var self = this
      self.$store.commit('PaymentManagement/setDelete', data)
    },
    submit () {
    },
    reset () {
  ...
  components: {
    'create-payment': require('components/PaymentManagement/PMAdd.vue').default,
    'update-payment': require('components/PaymentManagement/PMUpdate.vue').default,
    'delete-payment': require('components/PaymentManagement/PMDelete.vue').default
  }
}
</script>
