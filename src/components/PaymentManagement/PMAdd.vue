...
export default {
  ...
  async created () {
     ...
     self.data.student_id = self.student_data.id
  },
  methods: {
    submit () {
      var self = this
      ...
      self.$store.commit('PaymentManagement/setPost', self.data)
      self.$store.dispatch('PaymentManagement/createData')
        .then(res => {
          // self.showNotif()
          ...
          self.$store.commit('PaymentManagement/pushList', res.data)
          self.reset()
    ...
    reset () {
      var self = this
      self.$store.commit('PaymentManagement/setPost', '')
      self.$emit('close')
    },
    showNotif () {
      var self = this
