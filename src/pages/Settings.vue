    ...
    <div class="my-card">
      ...
      >
        ...
        <template v-slot:body="props">
          <q-tr :props="props">
            <q-td align="left">
              {{ props.row.miscellaneous }}
            </q-td>
            <q-td align="left">
              {{ props.row.tuition_fee }}
            </q-td>
            <q-td align="left">
              {{ props.row.nstp }}
            </q-td>
    ...
    <div>
      <div class="row q-gutter-md row items-start">
        <q-page-sticky position="bottom-left" :offset="[30, 15]">
          <q-btn fab icon="recent_actors" to="/EnrollmentManagement" color="black">
...
export default {
  data () {
    ...
    return {
      ...
      columns: [
        ...
        { name: 'miscellaneous', label: 'Miscellaneous', field: 'miscellaneous', align: 'left' },
        { name: 'tuition_fee', label: 'Tuition Fee', field: 'tuition_fee', align: 'left', sortable: true },
        { name: 'nstp', label: 'NSTP', field: 'nstp', align: 'left', sortable: true },
  ...
  async created () {
    ...
    await self.$store.dispatch('Settings/fetchData')
      .then(res => {
        self.$store.commit('Settings/setList', res.data)
        self.data_list = self.$store.getters['Settings/getList']
      }).catch(err => {
        console.log(err)
      })
  ...
  methods: {
    getPayload (data) {
      var self = this
      self.$store.commit('Settings/setPut', data)
      self.$router.push('/SUpdate')
    },
    getPayloadDelete (data) {
      var self = this
      self.$store.commit('Settings/setDelete', data)
    }
  },
  components: {
    // 'employee-create': require('components/EREmployee/Create.vue').default,
    // 'setting-update': require('components/Settings/SettingUpdate.vue').default
