    ...
    <div class="my-card">
      ...
      >
        ...
        <template v-slot:top-right>
          <q-btn color="primary" icon-right="recent_actors" label="Enroll" :disable="create_button_disable" to="/EMAdd" />
        </template>
    ...
    <div>
      <div class="row q-gutter-md row items-start">
        <q-page-sticky position="bottom-left" :offset="[30, 15]">
          <q-btn fab icon="settings" to="/EnrollmentSettings" color="black">
...
export default {
  data () {
    ...
    return {
      ...
      columns: [
        ...
        // { name: 'student_id', label: 'Id', field: 'student_id', align: 'left' },
        { name: 'full_name', label: 'Name', field: 'full_name', align: 'left', sortable: true },
        { name: 'academic_year', label: 'Academic Year', field: 'academic_year', align: 'left', sortable: true },
        { name: 'trimester', label: 'Trimester', field: 'trimester', align: 'left', sortable: true },
        { name: 'year_level', label: 'Year Level', field: 'year_level', align: 'left', sortable: true },
  ...
  async created () {
    ...
    if (self.$store.state.user_info.user_group === 'Accounting' || self.$store.state.user_info.user_group === 'Student Assistant') {
    ...
    await self.$store.dispatch('EnrollmentManagement/fetchData')
      .then(res => {
        self.$store.commit('EnrollmentManagement/setList', res.data)
        self.data_list = self.$store.getters['EnrollmentManagement/getList']
      }).catch(err => {
        console.log(err)
      })
  ...
  methods: {
    getPayload (data) {
      var self = this
      self.$store.commit('EnrollmentManagement/setPut', data)
      self.$router.push('/EMUpdate')
    },
    getPayloadDelete (data) {
      var self = this
      self.$store.commit('EnrollmentManagement/setDelete', data)
    }
  },
  components: {
    ...
    // 'update-form': require('components/EnrollmentManagement/EMUpdate.vue').default,
    'delete-form': require('components/EnrollmentManagement/EMDelete.vue').default
  }
}
</script>
<style lang="sass">
